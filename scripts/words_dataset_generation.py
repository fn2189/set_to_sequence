"""
example run: python scripts/words_dataset_generation.py --n-train 10000 --n-val 2000 --n-test 200 --n-set 5 --max-length 25
"""
import numpy as np
import pickle 
import argparse
...
def main():
    ...
    X_train, Y_train, W_train = generate_set(args.n_train, args.n_set, args.max_length)

    X_val, Y_val, W_val = generate_set(args.n_val, args.n_set, args.max_length)
    
    X_test, Y_test, W_test = generate_set(args.n_test, args.n_set, args.max_length)
...
def generate_word(max_length):
    ...
    word_length = random.choice(range(max_length))
    word = ''
...
def generate_set(N, n_set, max_length):
    l_word = []
    l_enc = []
    for i in range(N):
        l_sub_word = []
        ...
        for j in range(n_set):
            encoding, word = generate_word(max_length)
